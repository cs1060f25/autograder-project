import type { UserProfile } from "./user-utils";
...
export async function getStudentAssignments(
  ...
  const stats = {
    ...
    pending: assignmentsWithSubmissions.filter(
      (a: any) => !a.submission || a.submission.status === "draft"
    ).length,
  };
...
export async function getTAAssignments(
  ...
  const courseIds = taAssignments.map((a: any) => a.course_id);
  ...
  const totalPending = assignmentsWithStats.reduce(
    (acc: number, a: any) => acc + (a.submissions_count - a.graded_count),
    0
  );
  const allGrades = assignmentsWithStats.flatMap((a: any) =>
...
export async function getInstructorData(
  ...
  const assignmentsWithStats = await Promise.all(
    (recentAssignments || []).map(async (assignment: any) => {
      const { data: submissions } = await supabase
        .from("submissions")
        .select("id, status, grade")
        .eq("assignment_id", assignment.id);
  ...
  const totalStudents = coursesWithStats.reduce(
    (acc: number, c: any) => acc + c.students_count,
    0
  );
  ...
  const allGrades = [
    ...coursesWithStats.flatMap((c: any) =>
      c.average_grade ? [c.average_grade] : []
    ),
    ...assignmentsWithStats.flatMap((a: any) =>
      a.average_grade ? [a.average_grade] : []
    ),
  ];
  const overallAverage =
