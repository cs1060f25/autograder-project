...
import { AlertCircle, CheckCircle, Loader2, Clock, User, FileText, AlertTriangle, History } from "lucide-react";
...
export function RegradeReviewModal({
  ...
  const handleApprove = async () => {
    console.log("Approve clicked", { requestId: request.id, resolutionNotes, pointsAwarded });
    setError(null);
  ...
  const handleDeny = async () => {
    ...
    if (!resolutionNotes.trim()) {
      setError("You must provide an explanation when denying a regrade request");
      return;
    }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Evaluate the student's regrade request and decide whether to approve or deny it.
          </DialogDescription>
        </DialogHeader>
        ...
        {success ? (
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-xs">{request.students?.email}</p>
                  </div>
                </div>
            ...
            <div className="border rounded-lg p-4 space-y-3">
              ...
              {auditMetadata?.rubric_criterion_text && (
                <div className="bg-blue-50 border-blue-200 border rounded p-3">
                  <p className="text-sm font-medium text-blue-900 mb-1">Criterion Description</p>
                  <p className="text-sm text-blue-800">{auditMetadata.rubric_criterion_text}</p>
                </div>
              )}
              ...
              <div className="grid grid-cols-3 gap-4 text-center">
                ...
                <div className="bg-red-50 rounded p-3">
                  ...
                  <p className="text-2xl font-bold text-red-700">{originalScore}</p>
                </div>
                <div className="bg-red-100 rounded p-3">
                  <p className="text-xs text-red-700 mb-1">Deduction</p>
                  <p className="text-2xl font-bold text-red-800">-{originalDeduction}</p>
                </div>
              </div>
            </div>
            ...
            <div className="border rounded-lg p-4 space-y-2">
              ...
              {loadingSubmission ? (
                <div className="text-center py-4">
                  ...
                  <p className="text-sm text-gray-500 mt-2">Loading submission...</p>
                </div>
              ) : submission ? (
            ...
            {/* TA Override History */}
            {auditMetadata?.ta_override_history && auditMetadata.ta_override_history.length > 0 && (
              <div className="border rounded-lg p-4 space-y-2">
                <h3 className="font-semibold text-lg flex items-center gap-2">
                ...
                <div className="space-y-2">
                  {auditMetadata.ta_override_history.map((override, idx) => (
                    <div key={idx} className="bg-yellow-50 border-yellow-200 border rounded p-3 text-sm">
                      ...
                      <p className="text-gray-700">
                        Changed score from <span className="font-semibold">{override.previous_score}</span> to{" "}
                        <span className="font-semibold">{override.new_score}</span>
                      </p>
                      {override.reason && (
                        <p className="text-gray-600 mt-1 italic">{override.reason}</p>
                      )}
                    </div>
                  ))}
                </div>
            ...
            {request.status === "pending" && (
              <div className="border-t pt-6 space-y-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="points-awarded">Points to Award (if approving)</Label>
                  <input
                    id="points-awarded"
                    type="number"
                    min="0"
                  ...
                  <p className="text-sm text-gray-500">
                    Enter the new score for this rubric item (0 to {maxPoints} points)
                  </p>
                </div>
            ...
            {request.status !== "pending" && (
              <div className="border-t pt-6 space-y-3">
                ...
                <div className={`rounded-lg p-4 ${
                  ...
                  {request.points_awarded !== null && request.points_awarded !== undefined && (
                    <p className="text-sm mb-2">
                      <span className="font-medium">Points Awarded:</span> {request.points_awarded}
                    </p>
                  )}
                  ...
                  {request.resolution_notes && (
                    <div className="mt-3 pt-3 border-t">
                      ...
                      <p className="text-sm whitespace-pre-wrap">{request.resolution_notes}</p>
                    </div>
                  )}
                </div>
              </div>
