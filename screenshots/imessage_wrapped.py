...
def load_messages(conn, handle_names, chat_info):
    ...
    messages = []
    for msg_id, is_from_me, date_val, handle_id, text, chat_id in cur.fetchall():
...
def compute_stats(messages, top_n=15, top_emoji_n=10):
    ...
    for msg in messages:
        ...
        if text_content:
            entry["text_chars"] += len(text_content)
    ...
    for entry in per_conv.values():
        ...
        if entry["text_messages"] > 0:
            entry["avg_length"] = entry["text_chars"] / entry["text_messages"]
        else:
            entry["avg_length"] = 0
          ...
          <table>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>{{ contact.total }}</td>
                <td>{{ '%.1f'|format(contact.avg_length) }}</td>
